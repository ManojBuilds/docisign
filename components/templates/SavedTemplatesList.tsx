    ...
    return (
      <Card className="text-center py-16 px-4 border-2 border-dashed bg-muted/5 shadow-none gap-0">
        ...
        <p className="text-muted-foreground max-w-sm mx-auto mb-8">
          Save any document as a reusable template to save time on recurring
          contracts.
        </p>
      </Card>
    );
    ...
    <Card className="flex flex-col h-full shadow-none border-slate-200 rounded-xl overflow-hidden">
      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-4">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="w-40 shadow-none border-slate-200"
          >
            <DropdownMenuItem asChild>
              <Link
                href={`/d/${template._id}/edit`}
      ...
      <CardFooter className="mt-auto pt-0">
        <UseTemplateDialog
          ...
          trigger={
            <Button
              variant="secondary"
              className="w-full text-xs font-semibold h-10 shadow-none"
            >
              Use Template
            </Button>
          }
