...
const competitors = [
  { name: "Boopsign", price: "$12", limit: "Unlimited", users: "1 user (No team support)" },
...
export default function CompetitorComparisonTable() {
  const yearlySavings = 66 * 12 - 12 * 12; // DocuSign annual minus Boopsign annual approx.

  return (
    <div className="overflow-x-auto bg-muted/30 py-16">
      <div className="container mx-auto px-4 text-center">
        <h2 className="text-3xl font-bold mb-4">
          Compare Boopsign vs DocuSign, PandaDoc & HelloSign
        </h2>
        <p className="text-muted-foreground max-w-2xl mx-auto mb-10">
          Boopsign offers a fast, simple, and affordable e-signature platform with unlimited document signing but currently does not support team accounts.
        </p>
        <Table className="min-w-[600px]">
          ...
          <TableBody>
            {competitors.map((competitor) => (
              <TableRow
                key={competitor.name}
                className={competitor.name === "Boopsign" ? "bg-primary/10 border border-primary" : undefined}
              >
                <TableCell className="font-medium flex items-center gap-2">
                  {competitor.name}
                  {competitor.name === "Boopsign" && (
